...
static char prod_name[8] = {0};
int msdc_emmc_get_prod_name(char *emmc_name) {

	strcpy(emmc_name,prod_name);
	return 0;
}
EXPORT_SYMBOL(msdc_emmc_get_prod_name);
#endif
...
#ifdef CONFIG_HQ_SYSFS_SUPPORT
	pr_err("%s: card prod_name is %s\n",__func__,card->cid.prod_name);
	strcpy(prod_name,card->cid.prod_name);
#endif
	return 0;
}
