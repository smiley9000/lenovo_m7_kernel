...
#define SDCARD_CD_PIN_GPIO 15 /*the number of cd pin gpio*/
...
static int emmc_get_emmc_name(char *emmc_name)
{
	...
	if (0 == msdc_emmc_get_prod_name(emmc_prod_name)) {
		/*adaptive emmc vendor info for AX3207-313 by hulitao at 2020/10/20 start*/
		...
		/*adaptive emmc vendor info for AX3207-313 by hulitao at 2020/10/20 end*/
		} else {
...
void get_emmc_info_work_func(struct work_struct *work) {
	...
	if (0 == emmc_get_emmc_name(emmc_name)) {
		hq_regiser_hw_info(HWID_EMMC,emmc_name);
	}
}
