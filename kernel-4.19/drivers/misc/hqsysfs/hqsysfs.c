...
static HW_INFO(HWID_CAPSENSOR,capsensor);
static HW_INFO(HWID_EFUSE,efuse);
static HW_INFO(HWID_BATTERY_VENDOR,battery_vendor);
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
static HW_INFO(HWID_GM_BATTERY_VENDOR,gm_battery_vendor);
...
struct pcba_info pcba[]={
	...
	{PCBA_M7_LTE_V1,"M7_LTE_V1"},
	{PCBA_M7_WIFI_V1,"M7_WIFI_V1"},
	{PCBA_M7_LTE_V2,"M7_LTE_V2"},
	{PCBA_M7_WIFI_V2,"M7_WIFI_V2"},
	{PCBA_M7_LTE_V3,"M7_LTE_V3"},
	{PCBA_M7_WIFI_V3,"M7_WIFI_V3"},
	{PCBA_M7_LTE_V4,"M7_LTE_V4"},
	{PCBA_M7_WIFI_V4,"M7_WIFI_V4"},
};
...
//	&hw_info_flashlight.attr,
	...
	&hw_info_capsensor.attr,
	&hw_info_efuse.attr,
	&hw_info_battery_vendor.attr,
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
	&hw_info_gm_battery_vendor.attr,
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 end */
//	&hw_info_alsps.attr,
...
static ssize_t huaqin_show(struct kobject *kobj, struct attribute *a, char *buf)
{
	...
	if(HWID_VER == hw->hw_id){
		...
		printk(KERN_INFO "%s pcba_config:%d\n",__func__,huaqin_pcba_config);
...
static unsigned int get_efuse_value_from_cmdline = 0;
...
static void show_efuse_value(void)
{
      if (get_efuse_value_from_cmdline == 1) {
          hq_regiser_hw_info(HWID_EFUSE,"1");
	  }else if (get_efuse_value_from_cmdline ==0){
          hq_regiser_hw_info(HWID_EFUSE,"0");
	  }
}
static int __init  get_efuse_value(char *str)
{
	if (strcmp("1", str) == 0) {
		get_efuse_value_from_cmdline = 1;
	}else if (strcmp("0", str) == 0){
        get_efuse_value_from_cmdline = 0;
	}
	return 0;
}
__setup("androidboot.sbc=", get_efuse_value);
