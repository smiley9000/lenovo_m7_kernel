...
#endif
/* Liverpool code for AX3207-347 by liruiju at 20-11-13 start */
#include "charger_class.h"
/* Liverpool code for AX3207-347 by liruiju at 20-11-13 end */
#include "mtk_leds_sw.h"
...
/* Liverpool code for AX3207-586 by gaominlong at 20201113 start */
#include <linux/gpio.h>
#include <asm-generic/gpio.h>
/* Liverpool code for AX3207-586 by gaominlong at 20201113 end */
...
#define LCM_ID0		499
#define LCM_ID1		500
/* Liverpool code for AX3207-586 by gaominlong at 20-11-13 end */
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_CUST_BLS_PWM:
		...
		return ((cust_set_brightness) (cust->data)) (level);
	/* Liverpool code for AX3207-347 by liruiju at 20-11-13 start */
	case MT65XX_LED_MODE_CHGIC:
		if (strcmp(cust->name, "red") == 0) {
			int set_led = 0;
			if (!level)
				set_led = 3;
			printk("%s:mode MT65XX_LED_MODE_CHGIC set_led:%d\n",__func__,set_led);
			return charger_dev_set_led_stat(set_led);
		}
	/* Liverpool code for AX3207-347 by liruiju at 20-11-13 end */
...
/* Liverpool code for AX3207-586 by gaominlong at 20201113 start */
	...
	if ((gpio_get_value(LCM_ID0) == 1 && gpio_get_value(LCM_ID1) == 0) ||
		(gpio_get_value(LCM_ID0) == 0 && gpio_get_value(LCM_ID1) == 0)) {
		level = level * 81 / 100;
	} else if (gpio_get_value(LCM_ID0) == 0 && gpio_get_value(LCM_ID1) == 1) {
		level = level * 94 / 100;
	}
	pr_err("current backlight level = %d\n", level);
/* Liverpool code for AX3207-586 by gaominlong at 20-11-13 end */
