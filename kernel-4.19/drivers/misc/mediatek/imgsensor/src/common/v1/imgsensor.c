...
static char mcam_info_summary[80] = "";
static char scam_info_summary[80] = "";
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		if(psensor_inst->sensor_idx == DUAL_CAMERA_MAIN_SENSOR - 1) {
			sprintf(mcam_info_summary, "main_camera: %s, id: %x", psensor_inst->psensor_name, sensorID);
			hq_regiser_hw_info(HWID_MAIN_CAM, mcam_info_summary);
		} else if(psensor_inst->sensor_idx == DUAL_CAMERA_SUB_SENSOR - 1) {
			sprintf(scam_info_summary, "sub_camera: %s, id: %x", psensor_inst->psensor_name, sensorID);
			hq_regiser_hw_info(HWID_SUB_CAM, scam_info_summary);
		}
		printk(" Sensor found ID = 0x%x num = %d\n", sensorID, SENSOR_FEATURE_CHECK_SENSOR_ID);
