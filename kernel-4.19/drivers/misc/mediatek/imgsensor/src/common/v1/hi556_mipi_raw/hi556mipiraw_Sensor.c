...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x095a, 0x492a);
...
unsigned int hi556_read_otpdata(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	int i = 0;
	LOG_INF("hi556_otp addr %d,size %d", addr, size);
	switch (size)
	{
	case 1:
		data[0] = hi556_otp.awb_lsc_station;//AWBAFConfig
		break;
	case 4:
		if (addr == 1){
			data[0] = hi556_otp.sensor_id ;
			data[1] = hi556_otp.lens_id;
		}else if (addr == 2){
			for(i = 0;i<size;i++)
			{
				data[i]  = hi556_otp.wb_data [i];
			}
		}else if (addr == 6){
			for(i = 0;i<size;i++)
			{
				data[i]  = hi556_otp.wb_data [i+4];
			}
		}
		break;
	case 1868:
		for (i = 0; i < size; i++) {
			data[i] = hi556_otp.lsc_data[i];
		}
		break;
	default:
		LOG_INF("HI556 hi556_otp_read failed \n");
		break;
	}
	return size;
}
...
static void otp_init_setting(void)
{
	...
	write_cmos_sensor_byte(0x0d02, 0x07);	
...
LOG_INF("HI556 addr = 0x%x \n", addr);
	if (addr == 0) {
		...
		hi556_otp.module_integrator_id = OTP_read_cmos_sensor(addr);//0x57
		hi556_otp.module_version = OTP_read_cmos_sensor(addr + 2);//01
		hi556_otp.software_version = OTP_read_cmos_sensor(addr + 3);//01
		hi556_otp.production_year = OTP_read_cmos_sensor(addr + 4);//01
		hi556_otp.production_month = OTP_read_cmos_sensor(addr + 5);//01
		hi556_otp.production_day = OTP_read_cmos_sensor(addr + 6);//14
		hi556_otp.awb_lsc_station = OTP_read_cmos_sensor(addr + 7);//0x0
		hi556_otp.AF_Flag = OTP_read_cmos_sensor(addr + 8);//0x0
		hi556_otp.sensor_id = OTP_read_cmos_sensor(addr + 10);//01
		hi556_otp.lens_id = OTP_read_cmos_sensor(addr + 11);//21
		hi556_otp.infocheck = OTP_read_cmos_sensor(0x411);
	}

	checksum = (hi556_otp.module_integrator_id + hi556_otp.module_version + hi556_otp.production_year + hi556_otp.software_version + hi556_otp.production_month + hi556_otp.production_day + hi556_otp.sensor_id + hi556_otp.lens_id) % 0xFF + 1;
	...
	LOG_INF("HI556 module_integrator_id = 0x%x, hi556_otp.awb_lsc_station =  0x%x, AF_Flag = 0x%x, module_version = 0x%x,software_version = 0x%x,production_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id = 0x%x, infocheck = 0x%x \n",\
		hi556_otp.module_integrator_id,hi556_otp.awb_lsc_station,hi556_otp.AF_Flag, hi556_otp.module_version ,hi556_otp.software_version, hi556_otp.production_year, hi556_otp.production_month, hi556_otp.production_day, hi556_otp.sensor_id, hi556_otp.lens_id, hi556_otp.infocheck);



	hi556_otp.WB_FLAG = OTP_read_cmos_sensor(0x432);
	...
	hi556_otp.LSC_FLAG = OTP_read_cmos_sensor(0x463);
	if (hi556_otp.LSC_FLAG == 0x01)
		lsc_start_addr = 0x464;
	else if (hi556_otp.LSC_FLAG == 0x13)
		lsc_start_addr = 0xbb4;
	else if (hi556_otp.LSC_FLAG == 0x37)
		lsc_start_addr = 0x1304;
	else
		LOG_INF("HI556 WB data invalid \n");
	...
	if (lsc_start_addr != 0) {
		...
		for (i = 0; i < 1868; i++) {
			...
			LOG_INF("HI556 lsc_data[%d] = 0x%x  ", i, hi556_otp.lsc_data[i]);
		}
		hi556_otp.infocheck = OTP_read_cmos_sensor(wb_start_addr + 1867);
		LOG_INF("HI556  LSC checksum = 0x%x\n",hi556_otp.infocheck);
	}


	write_cmos_sensor_byte(0x0a00, 0x00);	// stand by on
	mdelay(10);
	write_cmos_sensor_byte(0x004a, 0x00);
	write_cmos_sensor_byte(0x0d04, 0x00);
	write_cmos_sensor_byte(0x003e, 0x00);	// OTP mode off
	write_cmos_sensor_byte(0x004a, 0x01);
	write_cmos_sensor_byte(0x0a00, 0x01);	// stand by off
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				hq_regiser_hw_info(HWID_MAIN_CAM, "lianyi hi556-rear 5m");
				LOG_INF(
