...
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		flag += idx * GC5035_OTP_GROUP_FLAG_OFFSET; //add qx  
		switch (GC5035_OTP_GET_2BIT_FLAG(flag, 6)) {
		case GC5035_OTP_FLAG_EMPTY: {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			module_start_offset = GC5035_OTP_MODULE_DATA_OFFSET
				...
				(GC5035_OTP_MODULE_DATA_SIZE + 1 ));//add qx
			...
			if ((check % 255 + 1) ==
				...
				/* Module info */		
				module_info.Module_House_ID = info[0];
				module_info.Module_code = info[1];
				module_info.Module_version = info[2];
				module_info.Software_version = info[3];
				module_info.Sensor_ID = info[4];
				module_info.Lens_ID = info[5];
				module_info.VCM_ID = info[6];
				module_info.Year = info[7];
				module_info.Month = info[8];
				module_info.Day = info[9];
				module_info.Mirror_Status = info[10];
				module_info.AWB_LSC_station_number = info[11];

			

				cam_pr_debug("Module_House_ID = 0x%x\n",
					module_info.Module_House_ID);
				cam_pr_debug("Module_code = 0x%x\n",
					module_info.Module_code);
				cam_pr_debug("Module_version = 0x%x\n",
					module_info.Module_version);
				cam_pr_debug("Software_version = 0x%x\n",
					module_info.Software_version);
				cam_pr_debug("Sensor_ID = 0x%x\n",
					module_info.Sensor_ID);
			    cam_pr_debug("Lens_ID = 0x%x\n",
					module_info.Lens_ID);
				cam_pr_debug("VCM_ID = 0x%x\n",
					module_info.VCM_ID);
				cam_pr_debug("Mirror_Status = 0x%x\n",
					module_info.Mirror_Status);
				cam_pr_debug("AWB_LSC_station_number = 0x%x\n",
					module_info.AWB_LSC_station_number);	
	...
	return module_info.Module_House_ID;
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc5035_otp_identify();
				hq_regiser_hw_info(HWID_MAIN_CAM, "jinkang gc5035-rear 5m");
