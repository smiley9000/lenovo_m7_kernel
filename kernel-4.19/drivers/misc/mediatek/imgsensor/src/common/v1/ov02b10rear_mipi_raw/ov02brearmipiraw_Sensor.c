...
#include "ov02brearmipiraw_Sensor.h"
#define PFX "ov02brear_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV02B10REAR_SENSOR_ID,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh
			LOG_INF("OV02B10REARMIPI_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh
			LOG_INF("OV02B10REARMIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh
			LOG_INF("OV02B10REARMIPI_SetGain = %d",iReg);
		}
	}
	else
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				hq_regiser_hw_info(HWID_MAIN_CAM, "lianyi ov02b10-rear 2m");
...
UINT32 OV02B10REAR_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
