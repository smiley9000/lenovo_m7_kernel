...
extern struct mtk_charger_type* g_get_bc12_type(void);
extern void do_charger_detect(struct mtk_charger_type *info, bool en);
extern struct charger_device* g_get_charger_device(void);
...
{
	int i_ACA_type = 0;
	static int i_ACA_type_last = 8;
	int ADC_voltage2;
	struct mtk_charger_type* sinfo = g_get_bc12_type();
	primary_charger = g_get_charger_device();

	while (1) {
		wait_event(thub_polling_thread_wq, (thub_thread_timeout == true));
		thub_thread_timeout = false;
		mutex_lock(&thub_polling_mutex);
		if (sinfo && primary_charger) {
			hw_otg_reset_aca_type();
			ADC_voltage2 = t_hub_get_bat_voltage();
			pr_err("[%s]: ADC_voltage2 %d\n", __func__,  ADC_voltage2);
			i_ACA_type = hw_otg_get_aca_type();
			pr_err("[%s][%d] ACA_type now:%d last: %d\n", __func__, __LINE__, i_ACA_type, i_ACA_type_last);
/* Liverpool code for AX3207-366 by zhanghuan at 20-11-26 start */
			if (i_ACA_type == 4 && i_ACA_type_last == 0) {//插入T-hub供电
				charger_dev_enable_otg(primary_charger, false);
				do_charger_detect(sinfo, true);
				pr_err("[T-hub]:i_ACA_type:%d ,i_ACA_type_last:%d.\n", i_ACA_type, i_ACA_type_last);
			} else if (i_ACA_type == 0 && i_ACA_type_last == 4) {//拔出T-hub供电
				charger_dev_enable_otg(primary_charger, true);
				do_charger_detect(sinfo, false);
				pr_err("[T-hub]:i_ACA_type:%d ,i_ACA_type_last:%d.\n", i_ACA_type, i_ACA_type_last);
			} else if (i_ACA_type == 0 && i_ACA_type_last == 8) {//插入otg
				charger_dev_enable_otg(primary_charger, true);
				pr_err("[T-hub]:i_ACA_type:%d ,i_ACA_type_last:%d.\n", i_ACA_type, i_ACA_type_last);
			} else if (i_ACA_type == 8 && i_ACA_type_last == 0) {//拔出OTG
				charger_dev_enable_otg(primary_charger, false);
				pr_err("[T-hub]:i_ACA_type:%d ,i_ACA_type_last:%d.\n", i_ACA_type, i_ACA_type_last);
			} else if (i_ACA_type == 8 && i_ACA_type_last == 4) {//拔出带电T-hub
				charger_dev_enable_otg(primary_charger, false);
				do_charger_detect(sinfo, false);
				pr_err("[T-hub]:i_ACA_type:%d ,i_ACA_type_last:%d.\n", i_ACA_type, i_ACA_type_last);
			} else if (i_ACA_type == 4 && i_ACA_type_last == 8) {//插入带电T-hub
				charger_dev_enable_otg(primary_charger, false);
				do_charger_detect(sinfo, true);
				pr_err("[T-hub]:i_ACA_type:%d ,i_ACA_type_last:%d.\n", i_ACA_type, i_ACA_type_last);
			}
...
/* Liverpool code for AX3207-366 by zhanghuan at 20-11-26 end */
			...
			sinfo = g_get_bc12_type();
			pr_notice("%s: get sinfo failed %d.\n", __func__, sinfo);
			primary_charger = g_get_charger_device();
			pr_notice("%s: get primary charger device failed %d.\n", __func__, primary_charger);
		}
...
#if THUB_SUPPORT
		i_ACA_type = hw_otg_get_aca_type();
		pr_err("[%s][%d] ACA_type =%d\n", __func__, __LINE__, i_ACA_type);

		if (i_ACA_type == 0x4) {
			vbus_on = true;
			pr_err("[%s][%d] Is Y cable, ACA_Type = %d\r\n", __func__, __LINE__, i_ACA_type);
			return;
		} else {
			pr_err("[%s][%d] Not Y cable, ACA_Type = %d\r\n", __func__, __LINE__, i_ACA_type);
			hw_otg_reset_aca_type();
		}
#endif //THUB_SUPPORT
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 end */
...
#if THUB_SUPPORT
	hrtimer_start(&thub_kthread_timer,
	ktime_set(0, MSEC_TO_NSEC(SW_POLLING_PERIOD)),HRTIMER_MODE_REL);
#endif //THUB_SUPPORT
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 end */
}
