...
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 Begin*/
static int s_sar_status_report = 0;
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 end*/
...
ssize_t do_situactive_store(int handle, int en, size_t count)
{
	struct situation_context *cxt = situation_context_obj;
	/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 Begin*/
	int err = 0, index = -1;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	return do_situactive_store(handle, en, count);
}
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 end*/
...
ssize_t do_situbatch_store(int handle, int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs,
	size_t count)
{
	struct situation_context *cxt = situation_context_obj;
	int index = -1, err = 0;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	return do_situbatch_store(handle, flag, samplingPeriodNs, maxBatchReportLatencyNs, count);
}
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 end*/
...
static ssize_t situflush_show(struct device *dev,
	struct device_attribute *artr, char *buf)
{
	int len = 0;
...
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 Begin*/
static ssize_t sardata_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", s_sar_status_report);
}

static ssize_t saractive_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct situation_context *cxt = NULL;
	int s_len = 0;
	...
	cxt = situation_context_obj;
    s_len += snprintf(buf + s_len, PAGE_SIZE, "en:%d\n",
        cxt->ctl_context[sar].enable);

	return s_len;
}

static ssize_t saractive_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int enable = 0, err = 0;
	...
	enable = !!enable;
	return do_situactive_store(ID_SAR, enable, count);
}

static ssize_t sarbatch_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct situation_context *cxt = situation_context_obj;
	int s_len = 0;

    s_len += snprintf(buf + s_len, PAGE_SIZE, "batch(ns):%lld\n", cxt->ctl_context[sar].delay_ns);
	return s_len;
}

static ssize_t sarbatch_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int64_t samplingPeriodNs = 0, err = 0;

	err = kstrtos64(buf, 10, &samplingPeriodNs);
	if (err != 0){
		pr_err("%s param error: err=%d\n", __func__, err);
		return err;
    }
    return do_situbatch_store(ID_SAR, 0, samplingPeriodNs, 0, count);
}
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 end*/
...
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 Begin*/
DEVICE_ATTR_RO(sardata);
DEVICE_ATTR_RW(saractive);
DEVICE_ATTR_RW(sarbatch);
/*add sar sensor support factory mode for AX3207-115 by zhangwei at 2020/10/16 end*/
