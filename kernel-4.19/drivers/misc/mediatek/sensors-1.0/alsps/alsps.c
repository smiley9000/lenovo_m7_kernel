...
/*add p sensor adapter for AX3207-372 by zhangwei at 2020/11/2 Begin*/
	return snprintf(buf, PAGE_SIZE, "en: %d div: %d\n", cxt->ps_enable, div);
}
			      ...
			      char *buf)
{
	...
	cxt = alsps_context_obj;
	err = cxt->ps_data.get_data(&value, &status);
	pr_debug("ps get data value: %d status: %d\n", value, status);
	s_len += snprintf(buf + s_len, PAGE_SIZE, "value:%d, status:%d\n",value, status);
	return s_len;
}
/*add p sensor adapter for AX3207-372 by zhangwei at 2020/11/2 end*/
