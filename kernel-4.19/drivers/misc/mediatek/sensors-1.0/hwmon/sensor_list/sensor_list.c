...
#include <linux/fs.h>
/*add sar gsensor support hardware info mode for AX3207-483 by zhangwei at 2020/10/20 Begin*/
#include <linux/hqsysfs.h>
/*add sar gsensor support hardware info mode for AX3207-483 by zhangwei at 2020/10/20 end*/
...
static void sensorlist_get_deviceinfo(struct work_struct *work)
{
	...
	for (handle = accel; handle < maxhandle; ++handle) {
		...
		/*add sar gsensor support hardware info mode for AX3207-483 by zhangwei at 2020/10/20 Begin*/
		if (sar == handle) {
			hq_regiser_hw_info(HWID_CAPSENSOR, sensorlist_info[handle].name);
		}
		if (accel == handle) {
