...
extern int gtx_enter_proximity_mode(int mode);
/* if use  this type of enable , Psensor only enabled but not report inputEvent to HAL */
static int ps_enable_nodata(int en)
{
	int err = 0;
//	if(get_huaqin_pcba_config() <= 0x31 && en)
		fts_call_ps_flag =1;
//	else
//		fts_call_ps_flag =0;
	/* Enable proximity */
	/*add p sensor adapter for AX3207-372 by zhangwei at 2020/11/6 Begin*/
	if(fts_ps_flag){
		...
		gtx_enter_proximity_mode(en);
	}
	/*add p sensor adapter for AX3207-372 by zhangwei at 2020/11/6 end*/
	return err;
}
...
extern int gtx_ps_get_data(int *value, int *status);
...
int fts_proximity_readdata(struct i2c_client *client)
{
    ...
    fts_i2c_read_reg(client, 0x02, &regvalue1);
    if (regvalue == 0xC0) {
        /* close. need lcd off */
    ...
    if ((proximity_status != (int)fts_proximity_data.detect && regvalue != 0xAA && regvalue1 == 0) || ((regvalue1 == 0) && (proximity_flags == 1))){
