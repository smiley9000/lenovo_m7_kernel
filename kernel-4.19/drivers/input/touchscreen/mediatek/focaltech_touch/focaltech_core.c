...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    fts_i2c_write_reg(client, FTS_CMD_READ_ID, 1);
...
static int tpd_local_init(void)
{
    ...
    //}
#endif
    pr_err("[Wangtuo FTS][%s][%d] tpd_load_status = %d\n", __func__, __LINE__, tpd_load_status);
	...
	FTS_ERROR("[TPD] error touch panel driver.");
...
static void tpd_resume(struct device *h)
{
...
#if FTS_PSENSOR_EN
    if ((fts_proximity_resume() == 0)&&(ts_data->suspended == false)) {
        return;
    }
#endif
    ...
    fts_i2c_write_reg(ts_data->client, FTS_CMD_READ_ID, 1);
...
#if FTS_PSENSOR_EN
    fts_proximity_init();
    FTS_INFO("fts_proximity_init OK");
#endif
