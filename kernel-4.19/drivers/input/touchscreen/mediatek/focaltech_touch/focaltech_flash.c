...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
    ...
    FTS_INFO("read vendor_id from flash is 0x%x\n", vendor_id);
...
void tp_info_regiser(struct fts_ts_data *ts_data)
{
    struct i2c_client *client;
    int ret = 0;
    u8 ver = 0;
    ...
    fts_fwupg_get_ver_in_tp(client, &ver);
    FTS_INFO(" tp fw version %02x", ver);
    ...
    sprintf(tp_info_summary, "%s version: %x, vendor_id: %04x", ic, ver, vendor_id);
