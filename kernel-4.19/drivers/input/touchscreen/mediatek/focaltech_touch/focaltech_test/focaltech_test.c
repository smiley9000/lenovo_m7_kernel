...
static int fts_ito_test_show(struct seq_file *file, void *data){
    ...
    char buf[128] = {0};
    if (is_first_tp)
        strcpy(buf, FTS_INI_FILE_NAME);
    else
        strcpy(buf, FTS_INI_FILE_NAME_FT3427G);
    FTS_TEST_INFO("is_first_tp is %d\n", is_first_tp);
    ...
    if(test_data.test_item[FT5X46_ENTER_FACTORY_MODE].testresult == RESULT_PASS){
        test_result_bmp[I2C_COMMUNICATION] = 'P';
    }else{
        seq_printf(file, "0F-1F-2F-3F-4F\n");
    }
    //Rawdata test result
    if(test_data.test_item[FT5X46_RAWDATA_TEST].testresult == RESULT_PASS){
        test_result_bmp[RAW_DATA] = 'P';
    }else{
        test_result_bmp[RAW_DATA] = 'F';
    }

    //SCAP_CB test result
    if(test_data.test_item[FT5X46_SCAP_CB_TEST].testresult == RESULT_PASS){
        test_result_bmp[SCAP_CB] = 'P';
    }else{
        test_result_bmp[SCAP_CB] = 'F';
    }
    //SCAP_RAWDATA test result
    if(test_data.test_item[FT5X46_SCAP_RAWDATA_TEST].testresult == RESULT_PASS){
        test_result_bmp[SCAP_RAWDATA] = 'P';
    }else{
        test_result_bmp[SCAP_RAWDATA] = 'F';
    }

    //short test result
    if(test_data.test_item[FT5X46_WEAK_SHORT_CIRCUIT_TEST].testresult == RESULT_PASS){
        test_result_bmp[WEAK_SHORT_CIRCUIT] = 'P';
    }else{
        test_result_bmp[WEAK_SHORT_CIRCUIT] = 'F';
    }

    seq_printf(file,"0%c-1%c-2%c-3%c-4%c\n",
        test_result_bmp[I2C_COMMUNICATION],
        test_result_bmp[RAW_DATA],
        test_result_bmp[SCAP_CB],
        test_result_bmp[SCAP_RAWDATA],
        test_result_bmp[WEAK_SHORT_CIRCUIT]);
...
int fts_test_init(struct i2c_client *client)
{
    ...
    fts_ito_test_proc = proc_create(FTS_ITO_TEST, 0666, fts_android_touch_proc, &ito_test_ops);
    if(!fts_ito_test_proc)
        FTS_TEST_ERROR("create proc entry %s failed", FTS_ITO_TEST);
    else
        FTS_TEST_DBG("create proc entry %s succuss", FTS_ITO_TEST);
