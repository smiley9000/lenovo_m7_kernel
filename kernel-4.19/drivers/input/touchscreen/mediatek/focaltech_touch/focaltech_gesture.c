...
unsigned int fts_call_gesture_flag = 0;
/*****************************************************************************
...
static ssize_t fts_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off){
    s32 ret = 0;
    char buf_temp[4] = {'\0'};
    ...
    if (ret) {
        FTS_ERROR("copy_from_user failed.");
        return -EPERM;
    }
    fts_gesture_data.mode = simple_strtol(buf_temp, NULL, 10);

    //pr_err("get_huaqin_pcba_config() = 0x%x",get_huaqin_pcba_config());

   // if(get_huaqin_pcba_config() <= 0x31){

        if(fts_gesture_data.mode){
            fts_call_gesture_flag = 1;
        }else{
            fts_call_gesture_flag = 0;
        }
  //  }
    FTS_INFO("gesture enabled:%x, ret:%d", fts_gesture_data.mode, ret);
    return len;
}

static ssize_t fts_gesture_data_read(struct file *file, char __user * page, size_t size, loff_t * ppos){
    s32 ret = -1;
    char buf[4] = {'\0'};

    FTS_DEBUG("visit fts_gesture_data_read. ppos:%d", (int)*ppos);
    if (*ppos) {
        return 0;
    }
    sprintf(buf, "%d",fts_gesture_data.mode);
    ret = simple_read_from_buffer(page, size, ppos, buf, strlen(buf));
    FTS_DEBUG("Got the gesture data.");
    return ret;
}

static int fts_gesture_open(struct inode *node, struct file *flip) {
    FTS_DEBUG("gesture node is opened.");
    return 0;
}

static int fts_gesture_release(struct inode *node, struct file *filp) {
...
static const struct file_operations fts_fops = {
    ...
    .release = fts_gesture_release,
    .read = fts_gesture_data_read,
    .write = fts_gesture_data_write,
};
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    proc_entry = proc_create(GESTURE_NODE, 0666, NULL, &fts_fops);
    if (proc_entry == NULL) {
        FTS_ERROR("Couldn't create proc entry[GESTURE_NODE]!");
        return -1;  
    } else {
        FTS_INFO("Create proc entry[GESTURE_NODE] success!");    
    }
    fts_gesture_data.mode = DISABLE;
    FTS_FUNC_EXIT();
    return 0;
