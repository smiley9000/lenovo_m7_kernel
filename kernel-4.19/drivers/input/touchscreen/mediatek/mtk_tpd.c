...
pr_err("Lomen 0\n");
	...
	/*Distinguish between primary and secondary code for by hulitao at 20201216 start*/
	GPIO_ID0 = of_get_named_gpio(dev->of_node, "tp_id0_gpio", 0);
	GPIO_ID1 = of_get_named_gpio(dev->of_node, "tp_id1_gpio", 0);
	gpio_direction_input(GPIO_ID0);
	gpio_direction_input(GPIO_ID1);
	/*Distinguish between primary and secondary code for by hulitao at 20201216 end*/
...
/*Fix build errors,as some projects  cannot support these apis while bring up*/
	...
	/*Distinguish between primary and secondary code for by hulitao at 20201216 start*/
	if (gpio_get_value(GPIO_ID1) == 0) {
		tp_device_name = "fts_ts";
	} else if (gpio_get_value(GPIO_ID0) == 0 && gpio_get_value(GPIO_ID1) == 1) {
		tp_device_name = "gt9xx";
	} else
		tp_device_name = "other";

	TPD_DMESG("tp_driver_name=%s; ID0=%d; ID1=%d;\n", tp_device_name,
			  gpio_get_value(GPIO_ID0), gpio_get_value(GPIO_ID1));

	/*Distinguish between primary and secondary for by hulitao at 20201216 end*/
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
		if (tpd_driver_list[i].tpd_device_name != NULL && tp_device_name != NULL) {
			if (strcmp(tpd_driver_list[i].tpd_device_name, tp_device_name) == 0) {
