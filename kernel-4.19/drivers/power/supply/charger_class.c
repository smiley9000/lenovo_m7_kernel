...
int charger_dev_set_led_stat(int ctrl)
{
	struct charger_device *charger_dev = NULL;

	charger_dev = get_charger_by_name("primary_chg");
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->set_led_stat)
		return charger_dev->ops->set_led_stat(charger_dev, ctrl);

	pr_err("%s: get charger device error!\n",__func__);
	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_led_stat);
