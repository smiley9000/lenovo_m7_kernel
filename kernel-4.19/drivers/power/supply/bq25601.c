...
static int bq25601_set_led_stat(struct charger_device *chg_dev, int ctrl)
{
	//struct bq25601_info *bq = dev_get_drvdata(&chg_dev->dev);

	bq25601_set_stat_ctrl(ctrl);
				   ...
				   bool en)
{
	...
	if (en) {
		...
		if (g_hiz_mode)
		{
			...
			pr_info("[%s] Enter hiz mode.\n", __func__);
		} else {
			bq25601_set_en_hiz(0x0);
			pr_info("[%s] Exit hiz mode.\n", __func__);
		}
		bq25601_set_chg_config(en);
...
static int bq25601_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	pr_info("%s en = %d\n", __func__, en);
	if (en) {
		/* AX3207 code for M7RSMT-135 by wangchao at 20201210 start */
		bq25601_set_en_hiz(0x0);
		/* AX3207 code for M7RSMT-135 by wangchao at 20201210 end */
		bq25601_set_chg_config(0);
		bq25601_set_otg_config(1);
		/* AX3207 code for ANDYSEVEN-1166 by wangqi at 20201221 start */
		//bq25601_set_watchdog(0x3);	/* WDT 160s */
		/* AX3207 code for ANDYSEVEN-1166 by wangqi at 20201221 end */
	} else {
...
static int bq25601_enable_ship_mode(struct charger_device
    *chg_dev, bool en)
{
    ship_mode_flag = true;
	return 0;
...
static unsigned int charging_hw_init(void)
{
	...
	/* AX3207 code for AX3207-382 by zhanghuan at 20201020 start */
	bq25601_set_batfet_disable(0x0);
	/* AX3207 code for AX3207-382 by zhanghuan at 20201020 end */
	bq25601_set_en_hiz(0x0);
...
extern struct power_supply* g_get_bc12_type_fg(void);
...
{
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 start */
	struct power_supply* psy = g_get_bc12_type_fg();
	if (chg_dev == NULL)
		return -EINVAL;
...
static struct charger_ops bq25601_chg_ops = {
	...
	.enable_ship_mode = bq25601_enable_ship_mode,
	...
	/* Liverpool code for AX3207-347 by liruiju at 20-10-29 start */
	/* set red led */
	.set_led_stat = bq25601_set_led_stat,
	/* Liverpool code for AX3207-347 by liruiju at 20-10-29 end */
};
...
static void bq25601_charger_shutdown(struct i2c_client *client)
{
	...
	if( ship_mode_flag )
	{
		pr_info("-------------ship_mode start!!!-------------");
		bq25601_set_batfet_delay(1);
		/* AX3207 code for AX3207-382 by zhanghuan at 20201020 start */
		bq25601_set_batfet_disable(1);
		/* AX3207 code for AX3207-382 by zhanghuan at 20201020 end */
		ret = bq25601_read_byte(bq25601_CON7, &val);
		if (ret == 0) {
			pr_info("[bq25601] i2c ship_mode read error!!!\n");
			//return 1;
		}
		pr_info("[0x%x]=0x%x ", bq25601_CON7, val);

	}

	pr_info("-------------ship_mode end!!!-------------");
}
