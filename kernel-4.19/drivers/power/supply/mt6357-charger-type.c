...
struct mtk_charger_type *info1;
unsigned int hq_chrdet=0;
...
/* Liverpool code for AX3207-366 by zhanghuan at 20-11-26 start */
#define voltage_threshold		400//mv
#define voltage_otg			300
/* Liverpool code for AX3207-366 by zhanghuan at 20-11-26 end */
extern signed int t_hub_get_bat_voltage(void);
...
/* Liverpool code for AX3207-366 by zhanghuan at 20-11-26 start */
	...
	while (i--)
	{
		ADC_voltage = t_hub_get_bat_voltage();
		pr_err("[%s]: ADC_voltage %d, voltage_threshold %d",
				__func__,  ADC_voltage, voltage_threshold);
		if(ADC_voltage < voltage_threshold && ADC_voltage > voltage_otg)
			msleep(100);
		else
			break;
/* Liverpool code for AX3207-366 by zhanghuan at 20-11-26 end */
...
static int get_charger_type(struct mtk_charger_type *info)
{
	...
	if (hw_bc11_DCD(info)) {
		...
		type = POWER_SUPPLY_USB_TYPE_FLOAT_CHARGER;
	} else {
...
void do_charger_detect(struct mtk_charger_type *info, bool en)
{
	...
	/* Liverpool code for AX3207-366 by liruiju at 20-11-05 start */
		...
		pr_err("%s: type:%d!\n", __func__, info->type);
		pr_err("charger type: %d, Now is usb host mode. Skip detection\n",info->type);
#else
	...
	case POWER_SUPPLY_USB_TYPE_DCP:
		info->psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
		break;
/* AX3207 code for AX3207-605 by zhanghuan at 20201127 start */
	case POWER_SUPPLY_USB_TYPE_FLOAT_CHARGER:
		info->psy_desc.type = POWER_SUPPLY_TYPE_USB_FLOAT;
		break;
/* AX3207 code for AX3207-605 by zhanghuan at 20201127 end */
	}
...
/* AX3207 code for ANDYSEVEN-1301 by miaozhichao at 20201225 start */
#if IS_ENABLED(CONFIG_USB_MTK_HDRC)
	int timeout = 200;
#endif
	msleep(200);
...
/* AX3207 code for ANDYSEVEN-1301 by miaozhichao at 20201225 end */
...
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 start */
struct mtk_charger_type* charger_type_info;
...
struct mtk_charger_type* g_get_bc12_type()
{
	return charger_type_info;
}
EXPORT_SYMBOL_GPL(g_get_bc12_type);
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 end */
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 start */
struct power_supply* g_get_bc12_type_fg()
{
	return charger_type_psy;
}

EXPORT_SYMBOL_GPL(g_get_bc12_type_fg);
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 end */
static int mt6357_charger_type_probe(struct platform_device *pdev)
{
	...
	/* Liverpool code for AX3207-366 by liruiju at 20-11-05 start */
	charger_type_info = info;
	/* Liverpool code for AX3207-366 by liruiju at 20-11-05 end */
	/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 start */
	charger_type_psy = info->psy;
	/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 end */
	pr_notice("%s: done\n", __func__);
