...
/* AX3207 code for AX3207-334 by zhanghuan at 20201027 start */
#include <linux/hqsysfs.h>
#include "battery_id_adc.h"
/* AX3207 code for AX3207-334 by zhanghuan at 20201027 end */
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 start */
#include "thub_adc.h"
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 end */
/* AX3207 code for AX3207-729 by wangqi at 20201225 start */
unsigned int change_temp=0;
struct battery_data *bs_data1;
/* AX3207 code for AX3207-729 by wangqi at 20201225 end */
struct tag_bootmode {
	u32 size;
	u32 tag;
	u32 bootmode;
...
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
	POWER_SUPPLY_PROP_GM_STATUS,
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 end */
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		break;
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
	case POWER_SUPPLY_PROP_GM_STATUS:
		val->intval = bs_data->bat_status;
		break;
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 end */
...
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 start */
int charger_dev_is_charging_done(struct charger_device *chg_dev, bool *done);
struct charger_device *chg1_dev;
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 end */
...
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 start */
	bool chg_done = false;
/* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 endt */
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		ret = power_supply_get_property(chg_psy,
          ...
          /* Liverpool code for ANDYSEVEN-3305 by wangqi at 21-01-18 start */
		charger_dev_is_charging_done(chg1_dev, &chg_done);
		if(chg_done)
...
/* AX3207 code for AX3207-729 by wangqi at 20201225 start */
static int battery_prop_writeable(struct power_supply *psy,
				  enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_TEMP:
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		#if 0
		change_temp=val->intval;
		#endif
		#if 0
		if(change_temp==1){
			bs_data1->bat_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
			power_supply_changed(bs_data1->psy);
			printk("szw:not_charging\n");
		}else if(change_temp==2){
			bs_data1->bat_status = POWER_SUPPLY_STATUS_CHARGING;
			power_supply_changed(bs_data1->psy);
			printk("szw:charging\n");
		}else if(change_temp==3){
			bs_data1->bat_status = POWER_SUPPLY_STATUS_DISCHARGING;
			power_supply_changed(bs_data1->psy);
			printk("szw:discharging\n");
		}
		#endif
		break;
...
void battery_service_data_init(struct mtk_battery *gm)
{
	...
	bs_data->psd.property_is_writeable = battery_prop_writeable;
  	/* AX3207 code for AX3207-729 by wangqi at 20201225 end */
...
/* AX3207 code for AX3207-830 by wangqi at 20210105 start */
static int bat_temperature_times = 0;
/* AX3207 code for AX3207-830 by wangqi at 20210105 end */
...
/* AX3207 code for AX3207-830 by wangqi at 20210105 start */
	if(bat_temperature_times == 0){
		bat_temperature_times = 1;
		for(i = 0;i < COUNT_TEMP;i++){
			fg_temp[i] = bat_temperature_val;
			average = bat_temperature_val;
		}
	}else{
		for(i=0;i<(COUNT_TEMP-1);i++){
			fg_temp[i] = fg_temp[i+1];
			sum = sum + fg_temp[i];
		}
		fg_temp[COUNT_TEMP-1] = bat_temperature_val;
		sum = sum + fg_temp[COUNT_TEMP-1];
		average = sum/COUNT_TEMP;
	}
	bm_err("bat_temperature_val==%d,average==%d",bat_temperature_val,average);
	gm->tbat_precise = average;
	return average / 10;
/* AX3207 code for AX3207-830 by wangqi at 20210105 end */
...
int force_get_tbat(struct mtk_battery *gm, bool update)
{
	...
	/* AX3207 code for AX3207-729 by wangqi at 20201225 start */
	if(change_temp!=0){
		gm->tbat_precise = (change_temp*10);
		gm->cur_bat_temp = change_temp;
		return change_temp;
	}
	/* AX3207 code for AX3207-729 by wangqi at 20201225 end */
...
/* AX3207 code for AX3207-830 by wangqi at 20210105 start */
	bat_temperature_val = gm->tbat_precise/10;
/* AX3207 code for AX3207-830 by wangqi at 20210105 end */
...
static enum hq_batteryID hq_get_battery_id()
{
	int id_volt = 0;
	...
	id_volt = bat_id_get_adc_val();

	pr_err("[%s]bat_id_get_adc_num:%d, id_volt = %d\n",
		__func__, bat_id_get_adc_num(), id_volt);

	if ( id_volt>0 && id_volt < 1000) {
		id = HQ_BATTERY_SWD;
	} else if ( id_volt>=1000 && id_volt < 3000 ){
		id = HQ_BATTERY_NVT;
	} else {
...
/* AX3207 code for AX3207-334 by zhanghuan at 20201027 start */
	bm_err("[%s] start!\n", __func__);
	bat_id = hq_get_battery_id();

	if (bat_id == HQ_BATTERY_NVT){
		hq_regiser_hw_info(HWID_BATTERY_VENDOR,"L20D1P32:NVT");
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
		hq_regiser_hw_info(HWID_GM_BATTERY_VENDOR,"L20D1P32:NVT");
	}
	else if (bat_id == HQ_BATTERY_SWD) {
		hq_regiser_hw_info(HWID_BATTERY_VENDOR,"L20D1P32:Sunwoda");
		hq_regiser_hw_info(HWID_GM_BATTERY_VENDOR,"L20D1P32:Sunwoda");
	}
	else {
		hq_regiser_hw_info(HWID_BATTERY_VENDOR,"Unknown");
		hq_regiser_hw_info(HWID_GM_BATTERY_VENDOR,"Unknown");
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 end */
	}
/* AX3207 code for AX3207-334 by zhanghuan at 20201027 end */
/* AX3207 code for AX3207-310 by zhanghuan at 20201110 start */
	gm->battery_id = bat_id;
/* AX3207 code for AX3207-310 by zhanghuan at 20201110 end */
	fg_cust_data = &gm->fg_cust_data;
	fg_table_cust_data = &gm->fg_table_cust_data;
...
/* AX3207 code for AX3207-773 by miaozhichao at 20201230 start */
	static bool led_flag = false ;
	static bool pre_led_flag = false;
	static bool first_time = true;
/* AX3207 code for AX3207-773 by miaozhichao at 20201230 end */
...
/* AX3207 code for AX3207-773 by miaozhichao at 20201230 start */
	...
	if(first_time){
		if(led_flag == false){
			charger_dev_set_led_stat(3);
		}else{
			charger_dev_set_led_stat(0);
		}
		first_time = false;
	}else{
		if(led_flag != pre_led_flag){
			if(led_flag == false){
				charger_dev_set_led_stat(3);
			}else{
				charger_dev_set_led_stat(0);
...
unsigned int g_chg_en_flag = 1;
bool g_hiz_mode = false;
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 start */
struct charger_device *g_chg_dev_info;

struct charger_device* g_get_charger_device()
{
	g_chg_dev_info = get_charger_by_name("primary_chg");

	return g_chg_dev_info;
}
EXPORT_SYMBOL_GPL(g_get_charger_device);
/* Liverpool code for AX3207-366 by liruiju at 20-11-05 end */
static int Charger_enable_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = g_chg_en_flag;
...
/* Liverpool code for M7RSMT-199 by wangqi at 21-01-15 start */
extern bool hq_charging;
/* Liverpool code for M7RSMT-199 by wangqi at 21-01-15 end */
static int Charger_enable_set(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int val)
{
	struct battery_data *bs_data;
	...
	case 0:
		g_hiz_mode = false;
		charger_dev_enable(chg1_dev, false);
		break;
	case 1:
		g_hiz_mode = false;
/* Liverpool code for M7RSMT-199 by wangqi at 21-01-15 start */
		if(hq_charging == 1)
/* Liverpool code for M7RSMT-199 by wangqi at 21-01-15 end */
		charger_dev_enable(chg1_dev, true);
		break;
	case 2:
		g_hiz_mode = true;
		charger_dev_enable(chg1_dev, false);
	...
	if (g_chg_en_flag == 0 || g_chg_en_flag == 2)
		bs_data->bat_status = POWER_SUPPLY_STATUS_DISCHARGING;
	else if (hq_charging == 1 && g_chg_en_flag == 1 && (gm->chr_type != POWER_SUPPLY_USB_TYPE_UNKNOWN))
		bs_data->bat_status = POWER_SUPPLY_STATUS_CHARGING;

	battery_update(gm);
	/* battery_charging_control(CHARGING_CMD_ENABLE, &g_chg_en_flag); */
	bm_err("g_chg_en_flag = %d\n", g_chg_en_flag);
	return 0;
}
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
static int gm_capacity_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	struct battery_data *bs_data;
	...
	if (gm->fixed_uisoc != 0xffff)
		...
		*val = bs_data->bat_capacity;

	bm_debug("%s %d\n", __func__, *val);
	return 0;
}
static int gm_temp_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	gm->bs_data.bat_batt_temp = force_get_tbat(gm, true) * 10;
	*val = gm->bs_data.bat_batt_temp;
	bm_debug("%s %d\n", __func__, *val);
	return 0;
}

static int gm_voltage_now_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	struct battery_data *bs_data;
	...
	gauge_get_property(GAUGE_PROP_BATTERY_VOLTAGE,
			&bs_data->bat_batt_vol);
	*val = bs_data->bat_batt_vol * 1000;

	bm_debug("%s %d\n", __func__, *val);
	return 0;
}
static int gm_current_now_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gauge_get_int_property(GAUGE_PROP_BATTERY_CURRENT)
			* 100;
...
static int gm_present_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	struct battery_data *bs_data;
...
static struct mtk_battery_sysfs_field_info battery_sysfs_field_tbl[] = {
	...
	BAT_SYSFS_FIELD_RW(Charger_enable, BAT_PROP_CHARGER_ENABLE),
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 start */
	BAT_SYSFS_FIELD_RO(gm_capacity, BAT_PROP_GM_CAPACITY),
	BAT_SYSFS_FIELD_RO(gm_temp, BAT_PROP_GM_TEMP),
	BAT_SYSFS_FIELD_RO(gm_voltage_now, BAT_PROP_GM_VOLTAGE_NOW),
	BAT_SYSFS_FIELD_RO(gm_current_now, BAT_PROP_GM_CURRENT_NOW),
	BAT_SYSFS_FIELD_RO(gm_present, BAT_PROP_GM_PRESENT),
/* AX3207 code for M7RSMT-48 by zhanghuan at 20201117 end */
};
...
int battery_init(struct platform_device *pdev)
{
	...
	gm->sw_iavg_gap = 3000;
/* AX3207 code for ANDYSEVEN-3301 by wangqi at 20210112 start */
	gm->tbat_precise = 250;
/* AX3207 code for ANDYSEVEN-3301 by wangqi at 20210112 end */
