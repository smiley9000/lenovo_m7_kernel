...
extern unsigned int hq_chrdet;
extern struct battery_data *bs_data1;
...
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
	if (of_property_read_u32(np, "jeita_temp_above_t4_cc_current", &val) >= 0)
		info->data.jeita_temp_above_t4_cc_current = val;

	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_cc_current", &val) >= 0)
		info->data.jeita_temp_t3_to_t4_cc_current = val;

	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_cc_current", &val) >= 0)
		info->data.jeita_temp_t2_to_t3_cc_current = val;

	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_cc_current", &val) >= 0)
		info->data.jeita_temp_t1_to_t2_cc_current = val;

	if (of_property_read_u32(np, "jeita_temp_below_t0_cc_current", &val) >= 0)
		info->data.jeita_temp_below_t0_cc_current = val;
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 end */
...
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
    /* AX3207 code for AX3207-729 by wangqi at 20201229 start */
	if (info->battery_temp >= info->data.temp_t4_thres) {
    /* AX3207 code for AX3207-729 by wangqi at 20201229 end */
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 end */
		chr_err("[SW_JEITA] Battery Over high Temperature(%d) !!\n",
			info->data.temp_t4_thres);
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
		sw_jeita->sm = TEMP_HOT_LEVEL;
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 end */
		sw_jeita->charging = false;
	} else if (info->battery_temp >= info->data.temp_t3_thres) {
		/* control 45 degree to normal behavior */
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
		if ((sw_jeita->sm == TEMP_HOT_LEVEL)
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 end */
		    && (info->battery_temp
			>= info->data.temp_t4_thres_minus_x_degree)) {
...
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
		if (((sw_jeita->sm == TEMP_WARM_LEVEL)
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 end */
		     && (info->battery_temp
			 >= info->data.temp_t3_thres_minus_x_degree))
		    || ((sw_jeita->sm == TEMP_COOL_LEVEL)
			&& (info->battery_temp
			    <= info->data.temp_t2_thres_plus_x_degree))) {
			...
			chr_err("[SW_JEITA] Battery Normal Temperature between %d and %d !!\n",
				...
				info->data.temp_t3_thres);
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
			sw_jeita->sm = TEMP_NORMAL_LEVEL;
                  	/* AX3207 code for AX3207-729 by wangqi at 20201225 start */
			sw_jeita->charging = true;
                  	/* AX3207 code for AX3207-729 by wangqi at 20201225 end */
		}
	} else if (info->battery_temp >=info->data.temp_t1_thres) {
		if ((sw_jeita->sm == TEMP_COLD_LEVEL)
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 end */
		    && (info->battery_temp
			<= info->data.temp_t1_thres_plus_x_degree)) {
				chr_err("[SW_JEITA] Battery Temperature between %d and %d,not allow charging yet!!\n",
...
/* AX3207 code for AX3207-388 by zhanghuan at 20201111 start */
	if (sw_jeita->sm != TEMP_NORMAL_LEVEL) {
		if (sw_jeita->sm == TEMP_HOT_LEVEL) {
			sw_jeita->cv = info->data.jeita_temp_above_t4_cv;
			sw_jeita->cc = info->data.jeita_temp_above_t4_cc_current;
		}
		else if (sw_jeita->sm == TEMP_WARM_LEVEL) {
			sw_jeita->cv = info->data.jeita_temp_t3_to_t4_cv;
			sw_jeita->cc = info->data.jeita_temp_t3_to_t4_cc_current;
		}
		else if (sw_jeita->sm == TEMP_NORMAL_LEVEL) {
			sw_jeita->cv = 0;
			sw_jeita->cc = info->data.jeita_temp_t2_to_t3_cc_current;
		}
		else if (sw_jeita->sm == TEMP_COOL_LEVEL) {
			sw_jeita->cv = info->data.jeita_temp_t1_to_t2_cv;
			sw_jeita->cc = info->data.jeita_temp_t1_to_t2_cc_current;
		}
		else if (sw_jeita->sm == TEMP_COLD_LEVEL) {
			sw_jeita->cv = info->data.jeita_temp_t0_to_t1_cv;
			sw_jeita->cc = info->data.jeita_temp_below_t0_cc_current;
		} else
...
extern unsigned int g_chg_en_flag;
extern bool g_hiz_mode;
...
static enum power_supply_property charger_psy_properties[] = {
	...
	/* Liverpool code for AX3207-347 by liruiju at 20-10-29 start */
	POWER_SUPPLY_PROP_REDLED,
	/* Liverpool code for AX3207-347 by liruiju at 20-10-29 end */
};
