...
static int bat_id_get_adc_info(struct device *dev)
{
	...
	md_channel = iio_channel_get(dev, "hq-battery");
...
signed int battery_get_bat_id_voltage(void)
{
	...
	if (!g_md_adc_pdev)
		return -1;
	channel = iio_channel_get(&g_md_adc_pdev->dev, "hq-battery");

	ret = IS_ERR(channel);
	if (ret) {
		pr_err("fail to get iio channel 2 (%d)", ret);
		goto BAT_Fail;
	}
...
int bat_id_adc_probe(struct platform_device *pdev)
{
    ...
    pr_err("battery_id_adc probe start.\n");
	...
	g_md_adc_pdev = pdev;
    pr_err("battery_id_adc probe end.\n");
	return 0;
}
