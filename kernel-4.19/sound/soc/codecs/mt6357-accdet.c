...
static struct switch_dev accdet_uevent;
...
static void send_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_NO_MIC:
		...
		switch_set_state(&accdet_uevent, status==0 ? NO_DEVICE : cable_type);
...
static int accdet_probe(struct platform_device *pdev)
{
	...
	/* register switch to report cable type */
	accdet_uevent.name = "h2w";
	accdet_uevent.index = 0;
	accdet_uevent.state = 0;
	ret = switch_dev_register(&accdet_uevent);
	if (ret) {
...
static int accdet_remove(struct platform_device *pdev)
{
	...
	switch_dev_register(&accdet_uevent);
