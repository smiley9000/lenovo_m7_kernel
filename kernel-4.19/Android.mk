...
$(KERNEL_ZIMAGE_OUT): $(TARGET_KERNEL_CONFIG) $(KERNEL_MAKE_DEPENDENCIES)
	...
	python $(TOP)/prebuilts/build/check_kernel_size.py $(KERNEL_OUT) $(KERNEL_DIR) $(PROJECT_DTB_NAMES)
...
### DTB build template
$(warning dir=$(KERNEL_DIR)/arch/$(KERNEL_TARGET_ARCH)/boot/dts/   PLATFORM_DTB_NAME=$(PLATFORM_DTB_NAME))
MTK_DTBIMAGE_DTS := $(addsuffix .dts,$(addprefix $(KERNEL_DIR)/arch/$(KERNEL_TARGET_ARCH)/boot/dts/,$(PLATFORM_DTB_NAME)))
$(warning test MTK_DTBIMAGE_DTS=$(MTK_DTBIMAGE_DTS))
include $(KERNEL_ROOT_DIR)/prebuilts/build/build_dtbimage.mk

MTK_DTBOIMAGE_DTS := $(addsuffix .dts,$(addprefix $(KERNEL_DIR)/arch/$(KERNEL_TARGET_ARCH)/boot/dts/,$(PROJECT_DTB_NAMES)))
include $(KERNEL_ROOT_DIR)/prebuilts/build/build_dtboimage.mk
