...
//	inode->i_version++;
	atomic64_add(1,&inode->i_version);
//	inode->i_mtime = inode->i_atime = inode->i_ctime = ts;
	inode->i_mtime = inode->i_atime = inode->i_ctime = current_time(inode);
	/* timestamp is already written, so mark_inode_dirty() is unnecessary. */

	dentry->d_time = dentry->d_parent->d_inode->i_version.counter;
...
//	ts = CURRENT_TIME_SEC;
	...
	atomic64_add(1,&dir->i_version);
	dir->i_mtime = dir->i_atime = current_time(inode);
	if (IS_DIRSYNC(dir))
		(void) exfat_sync_inode(dir);
	else
		mark_inode_dirty(dir);
...
//	ts = CURRENT_TIME_SEC;
	...
	atomic64_add(1,&new_dir->i_version);
	...
	atomic64_add(1,&old_dir->i_version);
...
static int __init init_exfat(void)
{
	...
	printk(KERN_ERR "ljj debug exFAT: Version %s\n", EXFAT_VERSION);

	err = exfat_init_inodecache();
	if (err)
		goto out;
	printk(KERN_ERR "ljj debug register_filesystem before\n");
	err = register_filesystem(&exfat_fs_type);
	printk(KERN_ERR "ljj debug register_filesystem after\n");
	if (err)
		goto out;
...
static void __exit exit_exfat(void)
{
	...
	printk(KERN_ERR "ljj debug unregister_filesystem before\n");
	FsShutdown();
}
